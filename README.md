...
Places LIMIT and MARKET orders on testnet.binancefuture.com via Binance REST API. Every order is signed with HMAC-SHA256, logged to logs/, and printed to the terminal with a clean summary. Designed for learning API integration, order mechanics, and data engineering with live-feed market data.
...
- HMAC-SHA256 signing: All requests signed per Binance API spec
- System clock sync: API signatures require accurate system time
- Manual invocation: No background daemon loop — each order is a deliberate CLI call
- Structured logs: All orders logged with timestamp, symbol, side, type, quantity, price, order ID

---

## What I Learned Building This

Financial data is not clean. Market data has gaps, timestamps drift, and exchanges have downtime. Working with a live-feed API teaches data engineering discipline that no Kaggle dataset does: error handling is not optional, latency matters, and every request can fail.
